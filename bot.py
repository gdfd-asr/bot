    ...
    try:
        if response.lower() == 'назад':
            ...
            if (not db.subscriber_cotegory(response)):
                if await rer(response):
                    await state.update_data(cotegory=response)
                    await message.reply('Введите канал в который будут отправляться сообщения(не забудьте добавить бота в этот канал и написать /start)', reply=False)
...
async def normal_handler(event):
    ...
    if r in t:
        ...
        if re != None:
            ...
            await bot.send_photo(chat_id, open('photo.jpg', 'rb'),result_1, parse_mode='HTML')
